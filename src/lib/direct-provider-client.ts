...
function parseCustomHeaders(rawHeaders?: string) {
  ...
  for (const rawLine of lines) {
    ...
    if (!name || !value || BLOCKED_BROWSER_HEADERS.has(normalizedName))
      continue;
...
async function fetchStreamWithConnectionTimeout(
  ...
  try {
    ...
    if (error instanceof DOMException && error.name === "AbortError") {
      throw new Error(
        "Provider stream timed out before the response started or was cancelled.",
      );
    }
    throw error;
  }
...
function getRequestTimeout(provider: ProviderConfig) {
  ...
  return normalizeTimeout(
    settings.requestTimeoutMs ?? defaultGenerationSettings.requestTimeoutMs,
  );
}
